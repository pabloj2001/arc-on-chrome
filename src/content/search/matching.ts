...
import type { HostPath, TabItem, HistoryItem } from "../../shared/types";

// True when every token appears in the item's title or url (case-insensitive).
export function matchesQuery(item: { title?: string; url?: string }, tokens: string[]): boolean {
...
export function templateBase(template: string): HostPath | null {
...
export function underBase(u: string, base: HostPath): boolean {
  const hp = hostPath(u);
...
export function computeDomainScores(openTabs: TabItem[], historyItems: HistoryItem[]): Map<string, number> {
...
export function bestDomainMatch(value: string, domainScores: Map<string, number>): string | null {
