...
chips collapse to just their numbers, with only the **active** chip showing its name; **hover** a
collapsed chip, or select it (click / **Ctrl+N**), and its name slides in inline as the chip
smoothly expands — deselecting reverses the animation.
...
- While the pill is active, the result list shows your open tabs and history under the
  ...
  variants of one destination don't crowd out the rest: for a path template like
